...
def public_tests():
    ...
    for i, content in enumerate(contents):
        if not contains_num_with_tolerance(content, query_results[i], tolerance=tolerances[i]):
            print(TerminalColors.RED + f"Teste {i+1} Falhou." + TerminalColors.RESET,
                  "Esperado: ", query_results[i], "Consulta: ", queries[i])
        else:
            print(TerminalColors.GREEN + f"Teste {i+1} Passou." + TerminalColors.RESET,
                  "Esperado: ", query_results[i], "Consulta: ", queries[i])
            num_passed += 1
